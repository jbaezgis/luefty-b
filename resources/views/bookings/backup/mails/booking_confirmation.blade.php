...
<html lang="en">
    ...
    <body>
        <div class="container" style="background:">
            <div class="">
                <h4>{{__('Thank You!')}}</h4>
            {{ __('Your payment has been successfully accepted!')}}
            </div>
            <hr>
            <p></p>
            <p style="">{{ __('From') }} <strong>{{ $msg->fromcity->name  }}</strong> {{ __('To') }} <strong>{{ $msg->tocity->name  }}</strong></p>
            {{-- <p></p>
            <a href="https://dev.luefty.com/booking/search?auction_id={{$msg->auction_id}}&email={{$msg->email}}" target="_blank">{{__('Click here to see your auction')}} </a> --}}
            <p></p>
            ...
            <p></p>
            <p class="card-text">{{ __('Order total') }}: ${{ number_format($msg->order_total + $msg->extras->sum('total'), 2, '.', ',') }}</p>
            <hr>
            <div class="">
