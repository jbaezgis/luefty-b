...
<div class="box box-solid 
    @if ($auction->category->code == 'booking' or $auction->category->code == 'booking_auction')
        {{$auction->vehicleType->max_passengers < $auction->passengers ? 'box-warning' : 'box-primary'}}
    @else
        box-primary
    @endif
    ">
    <div class="box-header with-border">
        <h4 class="box-title">{{ __('Passengers')}} </h4>
    </div>
    <div class="box-body">
        <div class="row">
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <input type="number" min="0" class="form-control {{ $errors->has('adults') ? 'is-invalid' : '' }}" id="adults" name="adults" value="{{ old('adults', $auction->adults) }}" aria-describedby="phoneErrors" required>
                    <small id="adultsError" class="form-text text-danger">{{ $errors->first('adults') }} </small>
                    <div class="invalid-feedback">
            ...
            {{-- <div class="col-md-4">
                <div class="{{ $errors->has('adults') ? ' has-error' : ''}}">
                    ...
                    {!! Form::number('adults', null, ['class' => 'form-control input-number', 'required']) !!}
            ...
            <div class="col-md-4">
                <div class="{{ $errors->has('infants') ? ' has-error' : ''}}">
                    ...
                    {!! Form::number('infants', null, ['class' => 'form-control', 'min' => '0']) !!}
            ...
            <div class="col-md-4">
                <div class="{{ $errors->has('babies') ? ' has-error' : ''}}">
                    ...
                    {!! Form::number('babies', null, ['class' => 'form-control', 'min' => '0']) !!}
        ...
        @if ($auction->category->code == 'booking_auction' and $auction->vehicleType->max_passengers < $auction->passengers)
            <p>
                <i class="fa fa-info-circle text-warning" aria-hidden="true"></i> {{ __('The number of passengers exceeds the maximum passengers supported by the selected vehicle.')}} <br>
                <a href="{{url('select_vehicle/'.$auction->key.'/edit')}} " class="btn btn-warning btn-sm">{{__('Vehicle List')}} </a> 
            </p>
        @endif

        @if ($auction->category->code == 'booking' and $auction->vehicleType->max_passengers < $auction->passengers)
            <p>
                <i class="fa fa-info-circle text-warning" aria-hidden="true"></i> {{ __('The number of passengers exceeds the maximum passengers supported by the selected vehicle.')}} <br>
                <a href="{{url('select_vehicle/'.$auction->key.'/edit')}} " class="btn btn-warning btn-sm">{{__('Vehicle List')}} </a> 
            </p>
...
<div class="box box-solid box-primary">
    ...
    <div class="box-body">
        <div class="row">
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control datepicker2 {{ $errors->has('date') ? 'is-invalid' : '' }}" id="date" name="date" value="{{ old('date', $auction->date) }}" aria-describedby="dateErrors" disabled>
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <label for="airline">{{ __('Arrival Airline')}}</label>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="form-group">
                    <label for="more_information">{{ __('Enter name of hotel or address of drop off location, and other details')}}</label>
    ...
    <div class="box box-solid box-primary">
        ...
        <div class="box-body">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('want_to_arrive') ? 'has-error' : '' }}" id="want_to_arrive" name="want_to_arrive" value="{{ old('want_to_arrive', $auction->want_to_arrive) }}">
                            ...
                            <option value="120" {{ $auction->want_to_arrive == '120' ? 'selected' : '' }}>2 hours 00 min</option>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        <label for="return_more_information">{{ __('Enter Hotel or Address and other additional Information if different from arrival drop-off information')}}</label>
                        <textarea class="form-control {{ $errors->has('return_more_information') ? 'is-invalid' : '' }}" id="return_more_information" name="return_more_information" aria-describedby="return_more_informationErrors">{{ old('return_more_information', $auction->return_more_information) }}</textarea>
