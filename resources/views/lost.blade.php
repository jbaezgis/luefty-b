            ...
            <div class="box-body">
                ...
                <div class="table-responsive">
                    <table class="table">
                        <thead>
                            <tr>
                                <th>@sortablelink('from', trans('globals.from')) | @sortablelink('to', trans('globals.to'))</th>
                        ...
                        <tbody>
                        @foreach($bids as $bid)
                            @if ($bid->auction['deleted'] == 0 & $bid->auction['status'] == 'Closed' & $bid->won === 0)
                            <tr>
                                <td>
                                ...
                                <td>
                                    @if ($bid->auction['status'] === 'Closed')
                                        <span class="badge badge-pill badge-danger">@lang('globals.closed')</span>
                                    @else
                                        <span class="badge badge-pill badge-success">@lang('globals.open')</span>
                                    @endif

                                </td>
                                <td>
                                    @if ($bid->auction['status'] == 'Closed' & $bid->won === 0)
                                        <i class="fa fa-times text-danger" aria-hidden="true"></i>
                                    @endif
                                </td>
                                <td>
                                    {{-- @if ($bid->auction_id) --}}
                                        <a href="{{ url('/my-auctions/' . $bid->auction_id) }}" title="View User"><button class="btn btn-primary btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> Ver</button></a> 
                                    {{-- @else  --}}
