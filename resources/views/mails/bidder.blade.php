  ...
  <body>
    <div class="container" style="background:">
        <div class="text-center">
            <h4>{{__('Congratulations you won this Auction!')}}</h4>
        {{ __('Here you have all the details of the auction and agency info.')}}
        </div>
        <hr>
        ...
        {!! $msg->auction->passengers ? '<p>'. __('Passengers').': <strong>'.$msg->auction->passengers.'</strong></p>' : ''!!}
         ...
         {{-- <p style=""><strong>{{ __('Bid:')}}</strong></p> --}}
         <p>
             {{ __('Your bid') }}: <strong>${{ number_format($msg->bid, 2, '.', ',') }}</strong>
         </p>

        {{ __('Click')}} <a href="{{ url('/myauctions/'.$msg->auction->id) }}" class=""><strong>{{ __('here')}}</strong></a> {{ __('to see this auction')}}
