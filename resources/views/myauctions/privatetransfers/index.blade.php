...
<div class="container-title">
    <h1 class="page-title bg-primary">{{ __('My Auctions') }} - {{__('All')}} (<small>{{ __('List of all auctions you have created')}}</small>)</h1>
</div>
...
<div class="col-md-12 d-none d-lg-block d-xl-block">
        <div class="box box-solid">
            <div class="box-header with-border">
                <a href="{{ route('myauctions.create') }}" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="{{ __('Click to enter transfer data that you want to post as an Auction.') }}"><i class="fa fa-plus" aria-hidden="true"></i> {{ __('Create new auction') }}</a>
                <div class="box-tools">
                    <button class="btn btn-secondary" type="button" data-toggle="modal" data-target="#rules">{{__('Rules')}}</button>
                </div>
            </div>
            <div class="box-body">
                <div class="table-responsive">
                    <table id="auctions" class="table table-bordered">

                        <thead>
                            <tr>
                                {{-- <th></th> --}}
                                <th><span data-toggle="tooltip" data-placement="top" title="{{ __('Your service number') }}">{{ __('#') }}</span></th>
                                <th>
                                ...
                                <th><span data-toggle="tooltip" data-placement="top" title="{{ __('Category') }}">{{ __('Category') }}</span></th>
                                <th>
                                    <span data-toggle="tooltip" data-placement="top" title="{{ __('Date of service') }}">{{ __('Date') }}
                        ...
                        <tbody>
                            ...
                            {{-- end querys --}}
                                <tr data-toggle="collapse" data-target="#auction{{$item->id}}" class="clickable">
                                    ...
                                    <td>{{ date('F j, Y', strtotime($item->date)) }}, <strong>{{__('From time')}}</strong>: {{ date('g:i a', strtotime($item->from_time)) }} - <strong>{{__('To time')}}</strong>: {{ date('g:i a', strtotime($item->to_time)) }}</td>
                                    @elseif ($item->category->code == 'contract')
                                        <td><strong>{{__('From')}}:</strong> {{ date('F j, Y', strtotime($item->start_date)) }} - <strong>{{__('To')}}:</strong> {{ date('F j, Y', strtotime($item->end_date)) }}</td>
                                    @endif
                                    ...
                                    <td>
                                        @if ($item->changed === 1 & $item->status === 'Closed')
                                            <span class="badge badge-pill badge-success" data-toggle="tooltip" title="{{ __('This Auction is Accepted because you accepted a bid.') }}" data-placement="top">{{ __('Accepted')}}</span>
                                        @elseif ($item->changed === 1)
                                            <span class="badge badge-pill badge-secondary" data-toggle="tooltip" title="{{ __('This Auction is Changed because you updated its info') }}" data-placement="top">{{ __('Changed')}} </span>
                                        @else
                                            ...
                                            @else
                                                ...
                                                @else
                                                    <span class="badge badge-pill badge-light" data-toggle="tooltip" title="{{ __('This Auction do not have bids.') }}" data-placement="top">{{ __('No bid yet') }}</span>
                                                @endif
                                            @endif
                                        @endif
                                    </td>
                                        ...
                                        <td colspan="8" class="p-0">
                                            <div class="collapse" id="auction{{$item->id}}">
                                                ...
                                                <div class="d-flex flex-row">
                                                    {{-- @if ($item->category->code == 'private')
                                                        <div class="p-2">
                                                            <strong>{{ __('Details') }}</strong>: <span class="text-primary">{{ $item->vehicle['name']}}</span>
                                                        </div>
                                                    @endif --}}
                                                    @if ($item->category->code == 'private')
                                                        <div class="p-2">
                                                    ...
                                                    @if ($item->bids->count())
                                                        <div class="p-2">
                                                            <strong>{{ __('Current bid') }}</strong>: <span class="{{ $item->category_id === 1 ? 'text-primary' : 'text-success'}} ">$ {{ number_format($item->bids->min('bid'), 2, '.', ',') }}</span>
                                                        </div>
                                                        <div class="p-2">
                                                ...
                                                <div class="d-flex flex-row mb-3 border-bottom">
                                                    <div class="p-2">
                                                        ...
                                                        @if (auth()->user()->isAdmin == true)
                                                            {{ $bid->user->name }}
                                                        @elseif($item->status == 'Closed' and $bid->won == 1)
                                                            {{ $bid->user->name }}
                                                        @else
                                                            {{__('Bid')}} {{ $i++ }}
                                                        @endif
